<template>
  ...
  >
    <div class="selected" :class="{ open: open }" @click="toggleOpen">
        <text v-if="this.modelValue.length === 0">{{ $t('other.choose_option') }}</text>
        <div v-else class="selected-gradient-rect"
        :style="{ background: `linear-gradient(90deg, ${options[modelValue].join(',')})` }"></div>
      <img
        :src="arrowIcon"
        class="arrow"
    ...
    <transition name="fade">
      ...
      >
        ...
        >
          <text class="gradient-text" :class="{ 'selected-text': option === modelValue }">{{ option }}</text>
          <div class="gradient-rect" :class="{ 'selected-option': option === modelValue }"
          :style="{ background: `linear-gradient(90deg, ${options[option].join(',')})`}">
          </div>
        </div>
      </div>
    </transition>
...
.custom-select {
  ...
  text-align: left;
  outline: none;
  padding: 5px 15px 5px 5px; /* Отступ для стрелки */
...
.custom-select .items div.selected-option {
  border: 3px solid var(--selected-gradient);
  width: 144px;
  height: 13px;
}
